...
export class CharacteristicComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.characteristicService
                ...
                .subscribe(
                    (res: HttpResponse<ICharacteristic[]>) => this.paginateCharacteristics(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/characteristic'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ICharacteristic) {
        return item.id;
    }
    ...
    protected paginateCharacteristics(data: ICharacteristic[], headers: HttpHeaders) {
